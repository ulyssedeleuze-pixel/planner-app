# PlanMaster – TODO

- [x] Configurer le thème (couleurs Indigo)
- [x] Générer et intégrer le logo de l'application
- [x] Configurer les icônes de navigation dans icon-symbol.tsx
- [x] Créer le contexte/store des événements (EventsContext)
- [x] Créer les types TypeScript (Event, Reminder)
- [x] Créer les utilitaires de date
- [x] Développer CalendarScreen (vue mensuelle)
- [x] Développer WeekScreen (vue hebdomadaire)
- [x] Développer EventListScreen (liste des événements)
- [x] Développer RemindersScreen (gestion des rappels)
- [x] Développer EventDetailScreen (détail d'un événement)
- [x] Développer EventFormScreen (création/modification)
- [x] Configurer la navigation par onglets (4 onglets)
- [x] Intégrer la persistance AsyncStorage
- [x] Intégrer les notifications locales (expo-notifications)
- [x] Mettre à jour app.config.ts (nom, logo)
- [x] Sauvegarder le checkpoint final

## Phase 3 : Onglet Aventure (Gamification)

- [x] Générer le personnage pixelisé style dark fantasy
- [x] Créer le contexte/store pour le système de gamification (pièces, équipements, progression)
- [x] Créer les types TypeScript pour les équipements et le personnage
- [x] Développer l'écran principal de l'Aventure (affichage du personnage + équipements)
- [x] Créer la boutique d'équipements avec achat et upgrade
- [x] Intégrer le système de pièces (gain automatique à la complétion des tâches)
- [x] Ajouter les animations d'équipement sur le personnage
- [x] Ajouter l'onglet "Aventure" à la navigation
- [x] Tester le système de gamification end-to-end
- [x] Sauvegarder le checkpoint final


## Phase 3b : Système de Récompense Avancé

- [x] Restructurer les événements pour inclure durée estimée et temps réel
- [x] Implémenter le calcul de crédits basé sur la durée (1h = X crédits)
- [x] Ajouter le système de validation manuelle avant octroi des crédits
- [x] Créer l'écran de validation des tâches completées
- [x] Implémenter le bonus de semaine complète
- [x] Créer l'historique des récompenses gagnées
- [x] Ajouter les statistiques de performance (tâches completées, crédits gagnés, etc.)
- [x] Tester le système de récompense end-to-end
- [x] Mettre à jour le checkpoint final
